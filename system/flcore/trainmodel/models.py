...
class Digit5CNN(nn.Module):
    def __init__(self):
        ...
        self.encoder.add_module("bn1", nn.BatchNorm2d(64, track_running_stats=False))  # ✅ 添加 track_running_stats=False
        ...
        self.encoder.add_module("bn2", nn.BatchNorm2d(64, track_running_stats=False))  # ✅ 添加 track_running_stats=False
        ...
        self.encoder.add_module("bn3", nn.BatchNorm2d(128, track_running_stats=False))  # ✅ 添加 track_running_stats=False
        self.encoder.add_module("relu3", nn.ReLU())

        self.linear = nn.Sequential()
        self.linear.add_module("fc1", nn.Linear(8192, 3072))
        self.linear.add_module("bn4", nn.BatchNorm1d(3072, track_running_stats=False))  # ✅ 添加 track_running_stats=False
        ...
        self.linear.add_module("bn5", nn.BatchNorm1d(2048, track_running_stats=False))  # ✅ 添加 track_running_stats=False
...
class CNN7(nn.Module):
    def __init__(self, in_features=3, num_classes=10):
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(32, 32, kernel_size=3, padding=1, stride=1, bias=True),
            nn.ReLU(inplace=True)
        )
        ...
        # Layer 5: Conv2d + ReLU (这是FreeEagle论文中的Lsep层)
        ...
        # Layer 7: Flatten + Fully Connected
        # 对于CIFAR-10 (32x32): 32/4 = 8, 所以是 64*8*8 = 4096
        # 对于MNIST (28x28): 28/4 = 7, 所以是 64*7*7 = 3136
        self.layer7 = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(64 * 8 * 8, 512),  # 针对CIFAR-10，如果是MNIST改为64*7*7
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(512, num_classes)
        )
    ...
    def _calculate_flatten_dim(self):
        ...
        # 创建一个dummy输入来计算维度
        with torch.no_grad():
            dummy_input = torch.randn(1, 3, 32, 32)  # CIFAR-10默认
            ...
            # 更新最后一层的输入维度
            self.layer7[1] = nn.Linear(flatten_dim, 512)

    def forward(self, x):
        out = self.layer1(x)  # Layer 1: Conv+ReLU
        out = self.layer2(out)  # Layer 2: Conv+ReLU
        out = self.layer3(out)  # Layer 3: MaxPool
        out = self.layer4(out)  # Layer 4: Conv+ReLU
        out = self.layer5(out)  # Layer 5: Conv+ReLU (Lsep层)
        out = self.layer6(out)  # Layer 6: MaxPool
        out = torch.flatten(out, 1)  # Flatten
        out = self.layer7(out)  # Layer 7: FC
        return out

    def get_layer5_features(self, x):
        """获取Layer 5 (Lsep层) 的特征，用于FreeEagle检测"""
        out = self.layer1(x)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
        out = self.layer5(out)  # 返回第5层的特征图
        return out


# 针对不同数据集的版本
class CNN7_MNIST(CNN7):
    def __init__(self, num_classes=10):
        super().__init__(in_features=1, num_classes=num_classes)
        # 重新计算MNIST的维度
        with torch.no_grad():
            dummy_input = torch.randn(1, 1, 28, 28)  # MNIST: 1通道，28x28
            x = self.layer1(dummy_input)
            x = self.layer2(x)
            x = self.layer3(x)
            x = self.layer4(x)
...
class CNN7_CIFAR(CNN7):
    def __init__(self, num_classes=10):
        super().__init__(in_features=3, num_classes=num_classes)
        # 默认就是CIFAR配置
